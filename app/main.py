...
_predict_status = {"running": False, "last_date": None, "error": None, "phase": None}
...
async def predict_daily(web_search: bool = Query(True, description="是否启用联网搜索新闻")):
    """收盘后一键：先复盘昨日预测（含 AI 点评），再预测明日涨跌。后台异步执行。"""
    _require_cache_ready()
    if not zhipu_available():
        raise HTTPException(status_code=503, detail="未配置 ZHIPU_API_KEY 环境变量，无法调用 AI 模型。")
    if _predict_status["running"]:
        return {"status": "already_running", "message": "任务进行中，请稍候。"}

    def _job():
        _predict_status.update(running=True, phase="reviewing", error=None)
        try:
            review_pending()                                    # 复盘（含 AI 点评，约 20-30s）
            _predict_status["phase"] = "predicting"
            rec = run_daily_prediction(use_web_search=web_search)  # 预测（约 2-3 min）
            _predict_status["last_date"] = rec["date"]
        except Exception as exc:
            _predict_status["error"] = str(exc)
            print(f"复盘+预测任务失败：{exc}")
        finally:
            _predict_status["running"] = False
            _predict_status["phase"] = None

    threading.Thread(target=_job, daemon=True).start()
    return {"status": "started", "message": "复盘+预测已开始，约几分钟后完成。"}
...
@app.get("/api/predict/latest-review")
def predict_latest_review():
    """返回最近一次已复盘的预测完整记录（含 AI 点评）。"""
    import os, re
